...
class TraversabilityTrainer:
    def __init__(self, cfg_dir: str):
        ...
        self.vision_encoder, self.traversability_decoder = self.load_pretrained_model()
    ...
    def train_batch(self, data):
        ...
        self.traversability_decoder.train() 
        ...
        v_encoded_thermal, thermal_features = self.vision_encoder(thermal, return_features=True)
    ...
    def load_pretrained_model(self):
        ...
        print(f"{datetime.now():%Y-%m-%d %H:%M:%S} - Loading VisionEncoder and traversabilityDecoder. Training End-to-End... ")
        ...
        vision_encoder.apply(init_weights(**self.cfg.init_weights))

        traversability_decoder = TraversabilityDecoder(
            latent_size=self.cfg.model.rep_size,
            num_layers=self.cfg.model.num_layers_enc 
        ).to(self.device)
        traversability_decoder.apply(init_weights(**self.cfg.init_weights))
    ...
    def init_optimizer(self):
        ...
        params_to_optimize = list(self.vision_encoder.parameters()) +  list(self.traversability_decoder.parameters())
        # Optimizer
        if self.cfg.train_params.optimizer.lower() == "adamw":
            optimizer = optim.AdamW(params_to_optimize, **self.cfg.trav_decoder_adamw)

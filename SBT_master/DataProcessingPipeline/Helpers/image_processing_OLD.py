...
def process_depth_map(depth_map):
    ...
    for y in range(height):
        if y < split_point:
            ...
            processed_depth[y][empty_mask[y]] = 200  # 200 for slight grey CHange- 9/5/25 - 30 - Max depth in meters
...
def dense_map(Pts, n, m, grid, post_process=True):
    ...
    # --- Part 3: Metric-Safe Post-Processing ---
    if post_process:
        inpaint_depth = process_depth_map(out)
        # Edge-preserving smoothing
        depth_map_bilateral = cv2.bilateralFilter(inpaint_depth, d=9, sigmaColor=75, sigmaSpace=75)
        ...
        return depth_map_bilateral

    return out

def create_sky_mask_from_lidar(sparse_points, image_shape):
    """
    Creates a sky mask by finding the convex hull of the projected LiDAR points.

    Args:
        sparse_points (np.ndarray): Array of (x, y, depth) sparse points.
        image_shape (tuple): The (height, width) of the target image.

    Returns:
        np.ndarray: A boolean mask where True indicates the sky/invalid region.
    """
    height, width = image_shape[:2]
    
    # Get the 2D (x, y) coordinates of all valid LiDAR points
    pixel_coords = sparse_points[:, :2].astype(np.int32)
    
    # We need at least 3 points to form a polygon
    if len(pixel_coords) < 3:
        # If not enough points, assume no valid region (entire image is sky)
        return np.ones(image_shape[:2], dtype=bool)

    # 1. Calculate the convex hull, which creates a tight polygon around the data
    hull = cv2.convexHull(pixel_coords)

    # 2. Create a black mask and draw the filled hull polygon in white
    mask = np.zeros(image_shape[:2], dtype=np.uint8)
    cv2.fillConvexPoly(mask, hull, 255)

    # 3. Invert the mask. Now white (True) represents the sky/invalid area.
    sky_mask = (mask == 0)
